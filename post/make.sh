...
if [ $isProblemSolving = "y" ]; then
  read -p "질문 2.1 - 문제 해결을 위한 게시글이라면, 어떤 사이트의 문제인가요? (ex. BOJ, Programmers ...) 
--> " site
  read -p "질문 2.2 - 문제 해결을 위한 게시글이라면, 문제 번호는 무엇인가요? (ex. 1000, 1001 ...) 
--> " problemNumber
  read -p "질문 2.3 - 문제 해결을 위한 게시글이라면, 문제의 제목은 무엇인가요? " problemTitle
  read -p "질문 2.4 - 문제 해결을 위한 게시글이라면, 문제를 해결한 언어는 무엇인가요? (ex. C++, Java ...) 
--> " language
  read -p "질문 2.5 - 문제 해결을 위한 게시글이라면, 문제의 태그들은 무엇인가요? (ex. DP, BFS ...) 
--> " tags
...
EOF
  ...
  read -p "질문 3.1 - 문제 해결을 위한 게시글이 아니라면, 어떤 주제의 게시글인가요? (ex. Book,  ...) 
--> " category
  read -p "질문 3.2 - 문제 해결을 위한 게시글이 아니라면, 주제의 내용은 무엇인가요? (ex. CleanCode1, NodeJS3 ...) 
--> " content
  read -p "질문 3.3 - 문제 해결을 위한 게시글이 아니라면, 주제의 제목은 무엇인가요? (ex. [Book] Clean Code - 2주차 (3장) ...) 
--> " title
  read -p "질문 3.4 - 문제 해결을 위한 게시글이 아니라면, 주제의 카테고리들은 무엇인가요? (ex. Backend, Books, Clean Code ...) 
--> " categories
  read -p "질문 3.5 - 문제 해결을 위한 게시글이 아니라면, 주제의 태그들은 무엇인가요? (ex. clean code, book ...) 
--> " tags
